        ...
        <div className="color_palette">
          {color_context.color.map((c, ind) => (
            <div
              ...
              className={`colors ${
                selectedColorInd === String(ind - 1000) ? 'highlight' : 'small'
              }`}
...
export function DefaultColor() {
  ...
  const selecting_color = (event) => {
    ...
    if (selectedColorInd !== event.target.id) {
      // if different than what is selected, update.
      selectedColorInd = event.target.id;
      selected_color_context.set_selected_color(event.target.style.backgroundColor);
    } else {
    ...
    <>
      ...
      >
        <p
          ...
          title="Add color"
          onClick={() => {
            set_show_color_wheel(!show_color_wheel);
          }}
        >
          +
        </p>
        ...
        <div className="default_color_palette">
          ...
          {user_colors.map((color, ind) => (
            <div
              ...
              className={`colors ${
                selectedColorInd === String(ind + default_color_scheme.length)
                  ? 'highlight'
                  : 'small'
              }`}
              onClick={selecting_color}
      ...
      >
        ...
        <Modal.Body
          className="flex-row-center py-3"
          style={{ backgroundColor: 'hsl(194, 51%, 96%)' }}
        >
        ...
        <Modal.Footer className="flex-col-center" style={{ backgroundColor: '#DEEFF5' }}>
          <button
            onClick={add_color}
            className={`btn bg-light-grey ${color_error ? 'shake' : ''}`}
          >
            Add Color
          </button>
          <p style={{ color: 'red' }} hidden={!color_error}>
            {' '}
            Color Already In List
          </p>
        </Modal.Footer>
      </Modal>
    </>
