...
const SelectedColorContextIndProvider = ({ children }) => {
  ...
  return (
    <SelectedColorContextInd.Provider value={value}>{children}</SelectedColorContextInd.Provider>
  );
};
...
const IsProcessedContextProvider = ({ children }) => {
  ...
  return <IsProcessedContext.Provider value={value}>{children}</IsProcessedContext.Provider>;
};
...
export default function App() {
  const [show_alert, set_show_alert] = useState(
    JSON.parse(localStorage.getItem('show_alert')) === 'SHOW' ||
      !JSON.parse(localStorage.getItem('show_alert'))
  );
              ...
              <IsProcessedContextProvider>
                <ImageContextProvider>
                  <div className="app_container">
                    ...
                    <br />
                    <footer
                      className="flex-col-center p-2 border border-top shadows"
                      style={{ backgroundColor: '#DEEFF5', marginTop: 'auto' }}
                    >
                      <div className="w-100 flex-row-center px-3">
                        Nathnael Bekele &nbsp;{' '}
                        <a
                          href="https://www.linkedin.com/in/nathnael-bekele-2b240b257"
                          target="_blank"
                          rel="noreferrer"
                        >
                          {' '}
                        ...
                        &nbsp;{' '}
                        <a href="https://github.com/natibek" target="_blank" rel="noreferrer">
                          {' '}
