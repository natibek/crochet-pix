...
export default function Display(){
    ...
    function neighbor_pixels(ind, size){
      ...
      function neigboring(x, y){
        ...
        if (x < dims.user_height - 1){ neighbors.push([x+1, y]) }
        if (y > 0){ neighbors.push([x, y-1]) }
        if (y < dims.user_width - 1){ neighbors.push([x, y+1]) }
        if (x < dims.user_height - 1 && y < dims.user_width - 1){ neighbors.push([x+1, y+1]) }
        if (x > 0 && y > 0){ neighbors.push([x-1, y-1]) }
        if (x < dims.user_height - 1 && y > 0 ){ neighbors.push([x+1,y-1])}
        if (x > 0 && y < dims.user_width - 1 ){ neighbors.push([x-1,y+1])}
